...
const fmtDate = (d) => d ? format(new Date(d), "dd MMM yyyy", { locale: es }) : "—";
...
export default function Historial() {
    ...
    const filteredNuevos = followers
        .filter((r) => r.scraped_at?.slice(0, 10) >= start && r.scraped_at?.slice(0, 10) <= end)
        .filter((r) => (r.username || "").toLowerCase().includes(searchNuevos.toLowerCase()) || (r.full_name || "").toLowerCase().includes(searchNuevos.toLowerCase()));

    const filteredPerdidos = lost
        .filter((r) => r.fecha_perdida?.slice(0, 10) >= start && r.fecha_perdida?.slice(0, 10) <= end)
        .filter((r) => (r.username || "").toLowerCase().includes(searchPerdidos.toLowerCase()) || (r.full_name || "").toLowerCase().includes(searchPerdidos.toLowerCase()));
    ...
    return (
        <div className="hist-page fade-in">
            <div className="hist-header">
                <div>
                    ...
                    <p className="hist-subtitle">Base de datos histórica de usuarios ganados y perdidos</p>
                </div>
                <DateRangePicker start={start} end={end} onChangeStart={setStart} onChangeEnd={setEnd} />
            </div>

            {loading && <div className="hist-state">Cargando tablas de datos...</div>}
            ...
            {!loading && !noAccount && (
                <div className="hist-grid">
                    {/* CARD: NUEVOS */}
                    <Card style={{ padding: "0", display: "flex", flexDirection: "column", overflow: "hidden", border: "1px solid var(--border)" }}>
                        <div className="hist-table-header" style={{ background: "var(--accent-teal-dim)", borderBottom: "1px solid var(--border)" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                                <div style={{ background: "var(--accent-teal)", padding: 6, borderRadius: 8, display: "flex", color: "white" }}><UserPlus size={18} /></div>
                                <span style={{ fontWeight: 800, fontSize: 16, color: "var(--accent-teal)" }}>
                  Nuevos <span style={{ opacity: 0.7, fontWeight: 500 }}>({filteredNuevos.length})</span>
                </span>
                            </div>
                            <div className="hist-search-box" style={{ background: "var(--bg-card)" }}>
                                <Search size={14} color="var(--text-muted)" />
                                <input type="text" placeholder="Buscar..." value={searchNuevos} onChange={(e) => setSearchNuevos(e.target.value)} />
                            </div>
                        </div>
                        <div className="table-container">
                            <table className="data-table">
                                <thead>
                                <tr><th style={{ width: "50%" }}>Usuario</th><th>Fecha</th><th>Hora</th></tr>
                                </thead>
                                <tbody>
                                {filteredNuevos.length === 0 ? (
                                    <tr><td colSpan="3">
                                        <div className="hist-empty"><UserPlus size={32} color="var(--border)" style={{ marginBottom: 12 }} /><br />Sin registros encontrados</div>
                                    </td></tr>
                                        ...
                                        <td>
                                            <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                                                ...
                                                <div style={{ display: "flex", flexDirection: "column" }}>
                                                    <span style={{ fontWeight: 600, color: "var(--text-primary)", fontSize: 14 }}>{u.full_name || "—"}</span>
                                                    <span style={{ color: "var(--text-secondary)", fontSize: 12, fontFamily: "monospace" }}>@{u.username}</span>
                                                </div>
                                            </div>
                                        </td>
                                        <td style={{ color: "var(--text-secondary)", fontWeight: 500 }}>{fmtDate(u.scraped_at)}</td>
                                        <td className="font-code" style={{ color: "var(--text-muted)" }}>{fmtTime(u.scraped_at)}</td>
                                    </tr>
                                ))}
                                </tbody>
                            </table>
                    ...
                    {/* CARD: PERDIDOS */}
                    <Card style={{ padding: "0", display: "flex", flexDirection: "column", overflow: "hidden", border: "1px solid var(--border)" }}>
                        <div className="hist-table-header" style={{ background: "var(--accent-pink-dim)", borderBottom: "1px solid var(--border)" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                                <div style={{ background: "var(--accent-pink)", padding: 6, borderRadius: 8, display: "flex", color: "white" }}><UserMinus size={18} /></div>
                                <span style={{ fontWeight: 800, fontSize: 16, color: "var(--accent-pink)" }}>
                  Bajas <span style={{ opacity: 0.7, fontWeight: 500 }}>({filteredPerdidos.length})</span>
                </span>
                            </div>
                            <div className="hist-search-box" style={{ background: "var(--bg-card)" }}>
                                <Search size={14} color="var(--text-muted)" />
                                <input type="text" placeholder="Buscar..." value={searchPerdidos} onChange={(e) => setSearchPerdidos(e.target.value)} />
                            </div>
                        </div>
                        <div className="table-container">
                            <table className="data-table">
                                ...
                                {filteredPerdidos.length === 0 ? (
                                    <tr><td colSpan="3">
                                        <div className="hist-empty"><UserMinus size={32} color="var(--border)" style={{ marginBottom: 12 }} /><br />Sin registros encontrados</div>
                                    </td></tr>
                                    ...
                                    <tr key={i}>
                                        ...
                                        <td style={{ color: "var(--text-secondary)", fontWeight: 500 }}>{fmtDate(u.fecha_perdida)}</td>
                                        <td className="font-code" style={{ color: "var(--text-muted)" }}>{fmtTime(u.fecha_perdida)}</td>
                                    </tr>
                                ))}
                                </tbody>
                            </table>
        ...
        .hist-search-box { display: flex; align-items: center; gap: 8px; padding: 6px 12px; border-radius: var(--radius-md); border: 1px solid var(--border); }
        ...
        .data-table th { position: sticky; top: 0; background: var(--bg-card); padding: 14px 24px; font-size: 11.5px; font-weight: 700; color: var(--text-muted); text-transform: uppercase; letter-spacing: 0.8px; border-bottom: 1px solid var(--border); z-index: 10; }
        ...
        .hist-state { text-align: center; padding: 80px; color: var(--text-muted); font-size: 15px; font-weight: 500; line-height: 2; }
        @media (max-width: 1024px) { .hist-grid { grid-template-columns: 1fr; } .hist-page { padding: 20px; } }
      `}</style>
        </div>
    );
}