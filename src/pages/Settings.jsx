...
function GeneralSection() {
    ...
    async function saveName() {
        if (!name.trim()) return
        setLoading(true)
        try { await settingsAPI.updateProfile(name); flash(true, 'Nombre actualizado.') }
            ...
            <div className="set-card">
                <FieldRow label="Nombre completo" hint="Se muestra en la plataforma.">
                    <div style={{ display: 'flex', gap: 8 }}>
                        <input className="set-input" value={name} onChange={e => setName(e.target.value)} placeholder="Tu nombre" />
                        <button className="set-btn-primary" onClick={saveName} disabled={loading}>Guardar</button>
                    </div>
                </FieldRow>
...
function AccountsSection() {
    ...
    async function handleDisconnect() {
        try { await settingsAPI.disconnectInstagram(); setIgUsername(null) }
...
function SyncSection() {
    ...
    return (
        <div className="set-panel fade-in">
            <SectionHeader title="Sync automático" subtitle="Configura a qué hora se ejecuta el scraping diario automáticamente." />
        ...
        <div className="set-root fade-in">
            ...
            <div className="set-layout">
                <aside className="set-sidebar">
                    {SECTIONS.map(s => (
                        <button key={s.id} className={`set-nav-item ${active === s.id ? 'active' : ''}`} onClick={() => setActive(s.id)}>
                            {s.icon}{s.label}
                        </button>
                    ))}
                </aside>
